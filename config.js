...
config.enableUser = to_boolean(process.env.IDM_ENABLE_USERS, false);
...
config.session = {
  secret:
    process.env.IDM_SESSION_SECRET ||
    require('crypto')
      .randomBytes(20)
...
// Enable CORS
config.cors = {
  enabled: to_boolean(process.env.IDM_CORS_ENABLED, true),
  options: {
    /* eslint-disable snakecase/snakecase */
    origin: to_array(process.env.IDM_CORS_ORIGIN, '*'),
    methods: to_array(process.env.IDM_CORS_METHODS, [
      'GET',
      'HEAD',
      'PUT',
      'PATCH',
...
config.oauth2 = {
  ...
  refresh_token_lifetime:
    process.env.IDM_OAUTH_REFR_LIFETIME || 60 * 60 * 24 * 14, // Two weeks
...
config.database = {
  ...
  password: process.env.IDM_DB_PASS || 'root',
  username: process.env.IDM_DB_USER || 'root',
  database: process.env.IDM_DB_NAME || 'idm1',
...
config.external_auth = {
  ...
  password_encryption_key:
    process.env.IDM_EX_AUTH_PASSWORD_ENCRYPTION_KEY || undefined,
  password_encryption_opt: {
    ...
    iterations:
      process.env.IDM_EX_AUTH_PASSWORD_ENCRYPTION_OPT_ITERATIONS || 27500,
  },
  database: {
...
// Config themes
config.site = {
  title: process.env.IDM_TITLE || 'Rebuild Identity Manager',
  theme: process.env.IDM_THEME || 'rebuild',
};
...
config.eidas = {
  ...
  node_host:
    process.env.IDM_EIDAS_NODE_HOST ||
    'https://se-eidas.redsara.es/EidasNode/ServiceProvider',
  metadata_expiration:
    process.env.IDM_EIDAS_METADATA_LIFETIME || 60 * 60 * 24 * 365, // One year
};
